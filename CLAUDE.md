...
```sh
cargo build --release                  # rebuild the binary so it
                                       # matches the just-merged code
cargo test --release | tail -3         # belt-and-braces: confirm the
                                       # merged tree still passes
./target/release/recon --version       # sanity: report the new version
```
...
The fix is one `cargo build --release` after every push. Cheap, and
the alternative is investigating why `recon --version` lies later.

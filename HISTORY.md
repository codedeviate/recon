...
**`friendly_message` filter compatibility.** recon's `main.rs` runs every error through a filter that rewrites any error message containing "TLS" or "certificate" to the generic "TLS/certificate error" message — useful for hiding rustls handshake noise from end users, but it would silently swallow the new module's "rebuild with --features impersonate" rebuild hint and the "not yet implemented" stub. Both messages were carefully phrased without those substrings (using "fingerprint", "rquest", "impersonation profile" instead) so users actually see them. Caught during the dispatch-skeleton task, not at the end. Documented in `src/impersonate.rs` so future contributors don't inadvertently re-introduce TLS/certificate words in user-facing error strings.
